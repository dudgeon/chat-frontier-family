...
export const useChatDatabase = () => {
  ...
  const updateChatNameInDb = useCallback(
    async (chatId: string, userId: string, newName: string) => {
      try {
  ...
  const initializeSessionName = useCallback(
  ...
  const updateSessionTimestampInDb = useCallback(
  ...
  const deleteSessionFromDb = useCallback(
    async (sessionId: string, userId: string) => {
      try {
        // Remove messages first to avoid foreign key constraints
        await supabase
          .from("chat_messages")
          .delete()
